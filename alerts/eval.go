...
// woken reads omagihu themselves.
type Fire struct {
...
// Evaluate folds one sample through one trigger, updating its state and
// returning whatever tripped. A trigger that is spent, expired or has no sample
// returns nothing and its state is left alone: a missing sample is not a
// transition.
...
func crossing(t *Trigger, value float64, now time.Time, describe func(float64) string) []Fire {
	...
	// The first sample teaches. A value already past the bound is history.
