...
func Alarm(t Trigger, f Fire) string {
	...
	b.WriteString(" This message carries no values; read omagihu yourself with the omagihu_* MCP tools.")
	if t.Standing {
		fmt.Fprintf(&b, " It stays armed until %s; disarm with: omagihu-setup disarm %s.",
...
func deliverDesktop(desktop Notifier, t Trigger, f Fire) error {
	...
	return desktop("critical", "omagihu alarm", body)
}
