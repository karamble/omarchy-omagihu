// Package alerts holds armed watches over omagihu's own view: an agent says
// "wake me when this happens", goes away, and is woken with the reason it
// cared. The alarm carries no values, so whoever is woken reads omagihu itself.
package alerts
...
type State struct {
	...
	// AwaySince is when a becomes value last left its target. The re-arm has to
	// measure from there, not from the last change: the sample that brings the
	// value back is itself a change, and would otherwise reset the clock it is
	...
	// sample teaches, it never fires: a value already past a bound is history.
...
func DefaultPath() string {
	if dir := strings.TrimSpace(os.Getenv("XDG_CONFIG_HOME")); dir != "" {
		return filepath.Join(dir, "omagihu", "triggers.json")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(".config", "omagihu", "triggers.json")
	}
	return filepath.Join(home, ".config", "omagihu", "triggers.json")
}
