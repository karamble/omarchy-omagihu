...
// Edit replaces a trigger's terms. The question has changed, so its learned
// state is cleared and the next sample teaches, exactly as it did when the
// trigger was first armed. Identity, ownership and the arming time survive: an
// edited trigger is the same watch, asked differently.
