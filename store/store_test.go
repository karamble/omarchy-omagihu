...
func TestWriteThenRead(t *testing.T) {
	...
	if err := d.Write("accounts.json", want, 0o600); err != nil {
		t.Fatal(err)
	}
	got, err := d.Read("accounts.json", 0o600)
	if err != nil {
		t.Fatal(err)
	}
	...
	info, err := os.Stat(filepath.Join(d.Path(), "accounts.json"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestWriteLeavesNoTemp(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		if err := d.Write("accounts.json", []byte("{}"), 0o600); err != nil {
			t.Fatal(err)
		}
	}
	...
	if len(entries) != 1 {
		...
		t.Errorf("after five writes the directory holds %v, want only accounts.json", names)
	}
}
...
func TestSweepRemovesLegacyTemps(t *testing.T) {
	...
	for _, name := range []string{
		".accounts-1472528310.json", ".accounts-937282217.json",
		".triggers-abc.json", ".tmp-leftover",
	} {
	...
	n, err := d.Sweep(".accounts-", ".triggers-")
	if err != nil {
		t.Fatal(err)
	}
	if n != 4 {
	...
	if ok, _ := d.Exists("accounts.json"); !ok {
...
func TestReadRefusesWideMode(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "accounts.json"), []byte("{}"), 0o644); err != nil {
		t.Fatal(err)
	}
	_, err = d.Read("accounts.json", 0o600)
...
func TestReadRefusesSymlink(t *testing.T) {
	...
	if err := os.Symlink(secret, filepath.Join(dir, "accounts.json")); err != nil {
		t.Fatal(err)
	}

	if _, err := d.Read("accounts.json", 0o600); err == nil {
...
func TestWriteSurvivesDirectorySwap(t *testing.T) {
	...
	if err := d.Write("accounts.json", []byte(`{"token":"secret"}`), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	got, err := os.ReadFile(filepath.Join(base, "moved", "accounts.json"))
	if err != nil {
